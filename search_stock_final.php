<?php 
	...
	$CompanyName="";
	if(isset($_GET["company"]))  $CompanyName=urlencode(trim($_GET["company"]));
	...
	if(number_format((float)$stock_quote_xml->results->quote->LastTradePriceOnly,2,'.',',')==0.0)
	{
		...
		$error_text = $doc->createTextNode("Error In Creating XML");
		$error_text = $error->appendChild($error_text);
	...
	else
	{
	  ...
	  $name_text = $doc->createTextNode(htmlspecialchars($stock_quote_xml->results->quote->Name));
	  $name_text = $name->appendChild($name_text);
	  
	  $symbol = $doc->createElement('Symbol');
	  $symbol = $root->appendChild($symbol);
	  
	  $symbol_text = $doc->createTextNode(htmlspecialchars($stock_quote_xml->results->quote->Symbol));
	  ...
	  foreach($company_news_xml->channel->children() as $item)
	  {
	   if($item->getName()== 'item')
	   {
			...
			foreach($childNodes as $cch)
			{
				...
				{
					$title=htmlspecialchars($cch);	
				}
				
				if($cch->getName()=='link')
				{
					$link=htmlspecialchars($cch);	
				}	
			}
